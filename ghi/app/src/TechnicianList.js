...
const TechnicianList = () => {
  ...
  const handleDelete = async (employee_id) => {
    ...
    if (result) {
      setAlert(true);
      setAlertMessage(
        "Problem with delete, if technicians are assigned to appointments, please re-assign before delete."
      );
    } else {
      setAlert(true);
      setAlertMessage("Successfully Deleted!");
      const newTechnicians = technicians.filter(
        (technician) => technician.employee_id !== employee_id
      );
                ...
                <tr key={technician.employee_id}>
                  ...
                  <td className="align-middle px-3 text-center">
                    <button
                      ...
                      role="button"
                      onClick={() => handleDelete(technician.employee_id)}
                    >
                      Delete
                    </button>
                  </td>
